...
namespace Microsoft.Data.OData
{
    ...
    public class ODataSwaggerConverter
    {
        ...
        protected virtual void InitializeContainer()
        {
            ...
            if (!EdmModel.EntityContainers().Any())
            {
                return;
            }
            var container = EdmModel.EntityContainers().Single();
            var noKeys = container.EntitySets().Where(w => !w.ElementType.Key().Any()).Select(se => se.Name).ToList();
            ...
            foreach (var operationImport in container.FunctionImports())
            {
        ...
        protected virtual void InitializeOperations()
        {
            ...
            var container = EdmModel.EntityContainers().Single();
            foreach (var operation in EdmModel.SchemaElements.OfType<IEdmFunctionImport>())
            {
                // skip unbound operation
                if (!operation.IsBindable)
                {
                    continue;
                }
                ...
                if (boundType.TypeKind == EdmTypeKind.Entity)
                {
                    ...
                    foreach (var entitySet in container.EntitySets().Where(es => es.ElementType.Equals(entityType)))
                    {
